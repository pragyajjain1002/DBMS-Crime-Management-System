...
<body>
  ...
  <table style="margin-left: 2%;">
    ...
    <tr> <td>
      <button id="history" onclick="document.getElementById('id04').style.display='block'">View FIRs filed in your station</button>
    </td> </tr>

  </table>

  <table id='id04' style="display: none;">
    <tr>
      <th>FirID</th>
      <th>Lodger</th>
      <th>LodgeDate</th>
    ...
    <?php
      $sql = "select StationID from Officer, User where Officer.AadhaarID = User.AadhaarID and User.Username = '" . $_SESSION['username'] . "';";
      ...
      $sql = "select * from Fir where StationID = " . $StationID . ";";
      $result = $conn->query($sql);
      if ($result->num_rows > 0){
        while( $row = $result->fetch_assoc() ){
            $firid = $row['FirID'];
            ...
            $lodger = $row['Lodger'];
            $status = $row['Status'];
            echo "<tr><td>" . $firid . "</td><td>" . $lodger . "</td><td>" . $ldate . "</td><td>" . $descr . "</td><td>" . $status . "</td>";
            if ($status == 'Registered'){
              echo "<form method='post' action='./query_police.php'>";
              echo "<td> <input type='hidden' name='firid' value='". $firid. "'>";
              echo "<select name='newstatus'> <option value='Under Investigation' selected> Under Investigation</option></select> </td>";
              echo "<td> <button type='submit' name='upstatus' value='upstatus'>Update</button></td> </form>";
            }
            elseif ($status == 'Under Investigation'){
              echo "<form method='post' action='./query_police.php'>";
              echo "<td> <input type='hidden' name='firid' value='". $firid. "'>";
              echo "<select name='newstatus'> <option value='In Court'>In Court</option> <option value='Closed'>Closed</option></select></td>";
              echo "<td><button type='submit' name='upstatus' value='upstatus'>Update</button></td> </form>";
            }
            else{
   ...
   <?php
     if ($_SERVER["REQUEST_METHOD"] == "POST") {
       ...
       elseif ( !empty($_REQUEST['ocr']) ) {
        //  Wish to lodge an FIR
         echo "file fir <br/>";
        ...
        //  Update the Status of FIR.
        echo "Update Status";
        $firid = $_POST['firid'];
        $sql = "update Fir set Status = '" . $_POST['newstatus'] . "' where FirID = " . $firid .";";
        echo $sql;
        if (!mysqli_query($conn,$sql)){
          echo("Error description: " . mysqli_error($conn));
        }
